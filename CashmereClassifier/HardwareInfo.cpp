...
char HardDriveSerialNumber [MAX_IDE_DRIVES][1024];
char HardDriveModelNumber [MAX_IDE_DRIVES][1024];
int iCurrentDriverNum=0;
   ...
   for (drive = 0; drive < MAX_IDE_DRIVES; drive++)
   {
      ...
      else
      {
         ...
         {         
			 ...
			 if (0 == HardDriveSerialNumber [iCurrentDriverNum][0] &&
						//  serial number must be alphanumeric
			            //  (but there can be leading spaces on IBM drives)
				   (isalnum (serialNumber [0]) || isalnum (serialNumber [19])))
			 {
				strcpy (HardDriveSerialNumber[iCurrentDriverNum], serialNumber);
				strcpy (HardDriveModelNumber[iCurrentDriverNum], modelNumber);
				iCurrentDriverNum++;
				done = TRUE;
			 }
#ifdef PRINTING_TO_CONSOLE_ALLOWED
...
long getHardDriveComputerID (int index)
{
   ...
   for(int i=0; i<MAX_IDE_DRIVES; i++)
		strcpy (HardDriveSerialNumber[MAX_IDE_DRIVES], "");
   ...
   if (HardDriveSerialNumber [index][0] > 0)
   {
      char *p = HardDriveSerialNumber[index];

         //  ignore first 5 characters from western digital hard drives if
         //  the first four characters are WD-W
      if ( ! strncmp (HardDriveSerialNumber[index], "WD-W", 4)) 
         p += 5;
      for ( ; p && *p; p++)
      {
         if ('-' == *p) 
   ...
   id %= 100000000;
   if (strstr (HardDriveModelNumber[index], "IBM-"))
      id += 300000000;
   else if (strstr (HardDriveModelNumber[index], "MAXTOR") ||
            strstr (HardDriveModelNumber[index], "Maxtor"))
      id += 400000000;
   else if (strstr (HardDriveModelNumber[index], "WDC "))
      id += 500000000;
   else
      id += 600000000;
   ...
   printf ("\nHard Drive Serial Number__________: %s\n", 
           HardDriveSerialNumber[index]);
   printf ("\nHard Drive Model Number___________: %s\n", 
           HardDriveModelNumber[index]);
...
vector<string> GetAllHardDiskSerialNumber()
{
	vector<string> sSerialNumbersList;
...
//		done = ReadPhysicalDriveInNTUsingSmart ();
	...
	for(int i=0; i<MAX_IDE_DRIVES; i++)
	{
		if (HardDriveSerialNumber [i][0] > 0)
		{
			string str=HardDriveSerialNumber[i];
// 			str+="(";
// 			str+=HardDriveModelNumber[i];
// 			str+=")";
			sSerialNumbersList.push_back(str);
		}
	}

	return sSerialNumbersList;
}



std::string GetFirstHardDiskSerialNumber()
{
	vector<string> sSerialNumbersList = GetAllHardDiskSerialNumber();
	if(sSerialNumbersList.size())
		return *sSerialNumbersList.begin();
	else
		return "";
}
...
{
	vector<string> sMacAdress = GetAllMACAddress();
	if(sMacAdress.size())
		return *sMacAdress.begin();
	else
		return "";
}

vector<string> GetAllMACAddress()
{
	...
	DWORD MACaddress = 0;
	IP_ADAPTER_INFO* AdapterInfo=new IP_ADAPTER_INFO[16];
	IP_ADAPTER_INFO* OldAdress=AdapterInfo;

	// for up to 16 NICs
	DWORD dwBufLen = sizeof(IP_ADAPTER_INFO)*16;  // Save memory size of buffer
	...
	while(AdapterInfo)
	{
		...
		if(pAdapterInfo)
		{
			...
			char string [256];
			sprintf (string, "%02X%02X%02X%02X%02X%02X", pAdapterInfo->Address[0], pAdapterInfo->Address[1], 
				pAdapterInfo->Address[2], pAdapterInfo->Address[3], pAdapterInfo->Address[4], pAdapterInfo->Address[5]);
			str=string;
// 			str+="(";
// 			str+=pAdapterInfo->Description;
// 			str+=")";
		}
